...
export default function Online({
  ...
  params: { room: number; username: string };
}) {
  ...
  const [playerPercentInfo, setPlayerPercentInfo] = useState(0);
  ...
  const initGame = () => {
    if (!isReady) {
      socketio.emit("init_quick_game", { room });

      socketio.on("quick_board", (data) => {
  ...
  const handleInputChange = (
    ...
    if (solvedBoard[row][col] !== value) {
      ...
      // 35 is the default difficulty level in backend
      setPlayerPercentInfo(Math.floor((100 * (playerInfo + 1)) / 30));
  ...
  useEffect(() => {
    ...
    socketio.emit("get_players_info", {
      playerPercentInfo,
      username,
      room,
      isDone,
    ...
    socketio.on("player_info", (data) => {
      if (data.username !== username) {
        ...
        setOpponentInfo(data.playerPercentInfo);
      }
    });
  ...
  const countNumbers = (array?: string[][], value?: number) => {
    ...
    if (value === -1) {
      for (let n = 1; n <= 9; n++) {
        for (let i = 0; i < 9; i++) {
          for (let j = 0; j < 9; j++) {
            if (array && array[i][j] !== "0" && n === parseInt(array[i][j])) {
  ...
  useEffect(() => {
    ...
    socketio.emit("get_players_info", { playerInfo, username, room, isDone });
  ...
  return (
    <div className="py-16 flex justify-center bg mb-32">
      <ToastContainer
        position="top-center"
        autoClose={5000}
      ...
      <div>
        ...
        {!message && !isReady && (
          <h1 className="flex justify-center text-3xl font-bold mt-2">
            Are you ready?
          </h1>
        )}
        {!message && !isReady && (
      ...
      <div>
        {isReady && (
          <div>
            <div className="mx-3 flex justify-between text-3xl font-bold">
              <h1>Opponent</h1>
              <h1>You</h1>
            </div>
            <div className="mt-1 mx-3 flex justify-between">
              <h1>
                Name: <strong>{opponent}</strong>
              </h1>
              <h1>
            ...
            <div className="mx-3 flex justify-between">
              <h1>
                State: <strong>%{opponentInfo}</strong>
              </h1>
              <h1>
                State: <strong>%{playerPercentInfo}</strong>
              </h1>
            </div>
            <h1 className="mt-3 mb-3 flex justify-center">
              <div>
